...
├── ha_lock_provider/        ← this repository
...
This is a Python library project. The package source lives under `src/`
and tests under `tests/`. Run commands from the repository root using
`uv`:

- Run tests before committing: `uv run pytest tests/`
- Run linting before committing: `uv run ruff check src/ tests/`
- Run type checking before committing: `uv run mypy src/`
